...
##                print(self.__height)
                ...
                box = box.resize(self.canvas.size_win())
    ...
    def coord_from_img(self, x, y=None):
        ...
        width = round(self.__width * coeff[0])
        height = round(self.__height * coeff[1])
        x = np.round(np.multiply(x, coeff)).astype(np.int).tolist()
        output = x
...
class ImageCanvas(Canvas):
    ...
    def size_img(self, coord):
        ...
        s = [self.__img.width, self.__img.height]
        return s
    ...
    def get_coeff(self):
        coeff = [self.__img.width/self.__size[0], self.__img.height/self.__size[1]]
        return coeff
